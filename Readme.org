...
This app is primarily designed for native desktop platforms, and its WebAssembly (wasm) target has a limited feature set that's best suited for quick document sharing and editing. For now, desktop landscape mode is the only supported orientation (tested on Chrome):
